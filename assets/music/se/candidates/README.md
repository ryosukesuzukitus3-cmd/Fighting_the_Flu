...
このフォルダには採用中の効果音と比較用の候補を保存しています。
**`src/story/aliases.py` の `SE` から参照する音は、通常プレイで使用しています。**
採用品の唯一の定義はこのエイリアス表です。変更時はエイリアスの参照先を更新してください。
配布manifestは参照される音だけを同梱し、未採用候補を配布物から除外します。
元の候補ファイルは開発時の比較用にリポジトリへ残します。
