...
- メインエージェント（上位モデル）は計画・分解・統合・レビューに専念し、独立したサブタスク（調査・ドキュメント取得・定型実装・PR作成）は軽量モデルのサブエージェントへ委任する。
- Claude Code の場合: Agent ツールの `model` 指定で、調査＝haiku（Explore）、実装・文書＝sonnet を使う。opus はサブエージェントには基本使わない。
...
- タスクに必要な範囲を超えた機能追加・リファクタ・抽象化をしない。バグ修正に周辺の清掃は不要。将来の仮定要件のための設計をしない。
