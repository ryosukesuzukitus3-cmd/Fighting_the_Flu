...
- HUDは上端88pxの3列配置。HP・体温を優先し、有給（0日を含む）・スコア、武器・自機と先輩の強化在庫、レーザー状態を表示する。撃破数は結果画面で確認する
- 新規開始・最初からやり直しは `Game.start_new_run()` でスコア・有給・強化・物語を初期化する。第一章に入ったという理由だけでは初期化しない
- 継続は有給を1日減らして現在章を再開し、スコア・撃破数を維持する。HPを全回復し、武器・相棒・物語の状態を章開始時のスナップショットへ戻す。有給0日では継続不可
- ポーズ、強化選択、停止型会話中は戦闘進行とコンボ時間を止める。戦闘入力の受付と `BREAK CHANCE!` の表示条件を揃える

### 2.3 武器システム（グラディウス式スロット選択）

WeaponItem（W）を取得すると自機の**強化在庫**が1増え、相棒がいる場合は先輩用の在庫も1増える。
1プレイ中の初回W取得では戦闘を停止して選択画面を開き、自機と先輩をそれぞれ強化できることを案内する。
以後は在庫があるときにウェポン選択キー（初期値 **V**）で開く。
上段で自機、下段で先輩の候補を選び、最後の確認欄で決定してそれぞれの在庫を1消費する。強化可能な系統が未選択なら決定前にその系統へ戻る。戻る操作では消費しない。
レーザーとホーミングは**併用可能**で、メインLv2以降に選択できる。レーザーは装備後に設定のレーザーキー（初期値 **B**）で使用する。
