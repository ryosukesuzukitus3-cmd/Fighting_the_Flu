...
- HUDは上端88pxの3列配置。HP・体温を優先し、有給（0日を含む）・スコア、武器・自機と先輩の強化在庫、持駒・レーザー状態を表示する。撃破数は結果画面で確認する
...
| スロット | 内容 |
|---|---|
| speed_level | 自機の移動速度UP（+20%/段、最大5段＝2倍） |
...
- **体幹（ボス）**: 武器別の体幹ダメージ（レーザー＝ブレイカー、メイン＝中、ホーミング＝小）で
  ...
  1プレイ中の初回ダウン時のみ、カロナール先輩がダウン中の被ダメ2倍を説明する。
- **体温オーバーヒート（プレイヤー）**: レーザーの発射・継続で体温が上がり 39.9℃で「熱暴走」＝1.6秒レーザー停止。
  通常射撃・ホーミングは常に使え、通常弾へ切り替えて冷却できる。
  先輩の解熱弾Lvが冷却を強化（解熱鎮痛剤の物語的役割とメカニクスを一致させる）。
  ボスダウン中は冷却2倍＝ダウン中の全弾放出を推奨する損得構造。
- **持ち駒攻撃**: コンボ閾値の通過で歩/金/龍を獲得（最大3、取得順）。設定の持駒キー（初期値B、内部名 `bomb`）で1つ使い、駒の種類に応じた追尾弾を発射する。
  全画面の弾消しや発動時の全体HPダメージは行わない。ボス戦では体幹へ控えめな補助ダメージを与え、金・龍には短い無敵時間が付く。
  ポーズ・強化選択・停止型会話中など、戦闘操作を受け付けない場面では消費しない。S4ボスの持駒打ちと対になる。
- **症状悪化（エンレイジ）**: フェーズ長期化で攻撃間隔が線形圧縮（最大1.4倍）。熱温存の
  待ち戦法への対抗。ビームの予告時間と大技の後隙は短縮しない。Form3 対象外。
