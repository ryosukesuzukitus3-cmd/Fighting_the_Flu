...
レーザーとホーミングは**併用可能**で、メインLv2以降に選択できる。レーザーは装備後に設定のレーザーキー（初期値 **Space**）で使用する。
...
| スロット | 内容 |
|---|---|
| speed_level | 自機の移動速度UP（+12%/段、最大3段＝1.36倍） |
...
class Weapon:
    ...
    speed_level:  int   # 0〜3（+12%/段、最大1.36倍）
...
- **成長**: 主砲は正面の弾を残して範囲・連射を増やす。レーザーは正面の集中火力、
  追尾弾は2ダメージの補助攻撃。速度は最大1.36倍で、操作感の変化を抑える。
