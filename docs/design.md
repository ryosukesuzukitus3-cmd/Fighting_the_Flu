...
| EnemyCoughSprayer | 画面右前方に滞空し扇状射撃 | 高HPの中ボス枠、短い間隔で小型弾を散布 |
| EnemySporeSplitter | 画面右前方に滞空・撃破時に分裂 | 高HPの中ボス枠、倒すと小型胞子ポッドを放出 |
...
- HealItem / ExtraLifeItem 等は取得即適用
...
| アイテム取得 | `item_pickup.wav`（SE_ITEM） | 0.7 | 通常時・ボス撃破後の取得 |
...
|---------|------------|-----|----------|
| タイプライター | `type.wav`（SE_TYPE） | 0.16 | StageIntroScene / CutsceneScene / BlackholeScene |
