...
弾幕に加え、ボスごとに異なるギミックを持つ（`boss.py` の `_GIMMICKS`）:

| ステージ/形態 | ギミック | 内容 |
|---|---|---|
| Boss1 / Boss4 第一形態 | shield | 周期的に無敵シールド展開＋強反撃。解除中が大ダメージ猶予 |
| Boss2 / Boss4 第二形態 | weakpoint | 装甲がある間は被ダメ無効。装甲を割ると弱点露出で被ダメ2倍 |
| Boss3 | turrets | 地形上に砲台を召喚。健在中は被ダメ減、全滅でスタン（被ダメ増） |
| Boss4 第三形態 | （専用演出） | 反芻再生・フェイクアウト等のスクリプト演出 |

### 2.5 アイテム

- WeaponItem は取得すると在庫に加算され、V キーで選択UIを開いて消費（1回1選択）
- HealItem / ScoreItem 等は取得即適用
...
- ステージクリア条件: ボス撃破
- ステージ開始時: モノローグ（StageIntroScene）→ ラウンドSE → ゲーム開始
...
├── PrologueScene           橋渡し（StageIntroScene へ委譲）
├── StageIntroScene         ステージ前モノローグ（タイプライター）
...
| `WeaponItem`, `LaserItem`, `HomingItem` | entities/items/weapon_item.py | 武器スロット選択UI・レーザー/ホーミング追加（同ファイルに統合） |
| `ShieldItem` | entities/items/shield.py | バリア（弾を1発相殺） |
| `HealItem` | entities/items/heal.py | HP回復 |
| `ScoreItem` | entities/items/score_item.py | スコア+1000 |
| `ExtraLifeItem` | entities/items/extra_life.py | 残機+1 |
...
```python
class BossPhase:
    hp_threshold: float   # このフェーズに入るHPの割合（例: 0.5 = 50%以下）
    attack_pattern: str   # 攻撃パターンの識別子

class Boss:
    phases: list[BossPhase] = [
        BossPhase(hp_threshold=1.0, attack_pattern="spread"),
        BossPhase(hp_threshold=0.5, attack_pattern="laser"),
        BossPhase(hp_threshold=0.2, attack_pattern="rush"),
    ]
```
...
│   │   ├── prologue_scene.py           橋渡し（StageIntroScene へ委譲）
│   │   ├── stage_intro_scene.py        ステージ前モノローグ（タイプライター）
...
│   │       ├── shield.py               ShieldItem（バリア）
│   │       ├── heal.py                 HealItem（HP回復）
│   │       ├── score_item.py           ScoreItem（+1000点）
│   │       └── extra_life.py           ExtraLifeItem（1UP）
│   ├── stages/
...
|------------------|------------|------|
| タイトル画面 | `bgm/The_Final_Battle.mp3` | 既にそのBGMが流れている場合は再起動しない |
