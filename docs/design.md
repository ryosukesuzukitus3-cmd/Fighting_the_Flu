...
タイトル画面
    → プロローグ / 第一章のブリーフィング
        → チュートリアルの案内（各新規プレイの導入で実施・省略を選べる）
        → 第一章開始
            → ステージ進行（全4ステージ）
                → ボス戦
                → ステージクリア / 次章への会話
...
- 上下左右の移動（画面内制限あり）
- HPゲージ（最大100・回数制限なしの再挑戦）。被弾源ごとに被ダメージ量が異なる（`balance.py` の `PLAYER_DMG_*`：雑魚接触22 / 敵弾20 / ボス接触25 / 地形8）
- 被弾でHP減少、0でゲームオーバー
- 被弾後は一定時間無敵（点滅で表現）
- 地形（壁・岩・デブリ）に接触してもダメージ。弾は地形で跳ね返り、レーザーは地形で遮られる。破壊可能地形は一定ダメージで壊れてアイテムを落とすことがある
- HUDは上端88pxの3列配置。HP・体温を優先し、再挑戦の案内・スコア、武器・自機と先輩の強化在庫、レーザー状態を表示する。撃破数は結果画面で確認する
- 新規開始・最初からやり直しは `Game.start_new_run()` でスコア・死亡回数・強化・物語を初期化する。第一章に入ったという理由だけでは初期化しない
- 継続は章開始・道中難所前・ボス前・最終決戦の休息地点へ戻る。回数制限なし。HP全回復、武器・先輩・物語・スコア・撃破数・強化取得数・地形をその地点へ戻し、強化の再取得による増殖を防ぐ。ボスの導入会話は繰り返さない
- ポーズ、強化選択、停止型会話中は戦闘進行とコンボ時間を止める。戦闘入力の受付と `BREAK CHANCE!` の表示条件を揃える

### 2.3 武器システム（グラディウス式スロット選択）

WeaponItem（W）を取得すると自機の**強化在庫**が1増え、相棒がいる場合はWを2個取得するごとに先輩用の在庫が1増える。
1プレイ中の初回W取得では戦闘を停止して選択画面を開き、自機の強化を案内する。
以後は在庫があるときにウェポン選択キー（初期値 **V**）で開く。
上段で自機、下段で先輩の候補を選び、最後の確認欄で決定してそれぞれの在庫を1消費する。強化可能な系統が未選択なら決定前にその系統へ戻る。戻る操作では消費しない。
レーザーとホーミングは**併用可能**で、初期状態から選択できる。レーザーは装備後に設定のレーザーキー（初期値 **B**）で使用する。
HUDと強化画面の操作案内は現在のキー設定から取得する。HPゲージ制移行に伴い、被弾時の自動ダウングレードは廃止。

道中のW配置は章順に3/2/2/3個、1〜3章ボス撃破で各1個。全体の最大取得数は13。
メイン・速度・レーザー・追尾の全強化には20ポイント必要なため、配分を選ぶ。
ビリーの大量回復は回復1個へ変更。先輩の補給はHPが減っているときに発生し、休息地点間で合計15/25/35HPまで。
各章にはスクロール停止の難所があり、2基の攻撃源を倒して2波（4章は3波）を突破すると進む。
攻撃順は咳の三連射、横の二連射、扇弾と縦射線、縦射線の連続。放置による通過はできない。

#### メインウェポン

通常の強化はLv0〜4（single〜wide2）。Lv5のMEDICは通常の強化候補には含めず、最終決戦の先輩復帰時に `grant_medic()` で解禁する。
MEDICは3方向の貫通弾を発射する。
...
| 1 | rapid1 | 連射（発射間隔0.19s） |
| 2 | rapid2 | 超連射（発射間隔0.14s） |
| 3 | wide1 | 正面＋左右交互の斜め 2本発射 |
| 4 | wide2 | 正面＋斜め 3本発射 |
| 5 | medic | 先輩復帰時に解禁する3方向貫通弾（通常強化の対象外） |
...
| スロット | 内容 |
|---|---|
| speed_level | 自機の移動速度UP（初期140px/秒、+35px/秒/段、最大3段＝245px/秒） |
| laser_level | 連続レーザー（0=なし、1〜6段階で幅・威力が変化） |
| homing_level | ホーミング弾追加（0=なし、1〜7段階） |
| magnet_level | 旧状態との互換用（0〜3）。通常のマグネット強化は先輩の支援系統で行う |
...
| EnemyBilly | 低速直進 | HP高い、回復1個。強化ドロップは章の配置で指定 |
...
2026-09-17承認のデザイン方針は、ドット文字・余白・少数の差し色。共通配色は `src/scenes/meta_ui.py` に定義する。

- タイトルは暖色の暗い背景、中央5項目、選択下線の従来デザインを使用する。
- メニューは黒に近い単色背景、温白の文字、コーラルの選択カーソル。強化の選択済みはミント色と文字で区別する。カード枠・角丸・装飾罫線・背景グリッドは使わない。
- 戦闘HUDは背景パネルや列の区切りを置かず、文字に1pxの影を添える。HP・体温・ボスHP・体幹のゲージは輪郭なしの直線的な塗りで表す。
- 会話は2pxの角ばった外枠をひとつだけ使い、話者名を内側へ置く。本文と操作案内は余白で分ける。現在/総ページ数、残数、進行ドット、最終ページ専用の案内は表示しない。
- 総会話数は描画APIへ渡さない。全文表示と次へは文字送り状態で切り替え、台詞・操作キー・長文の読みやすさを保つ。

- タイトル画面
- チュートリアル（新規プレイの第一章前で案内。タイトルから練習だけを選ぶことも可能）
- ステージイントロ（各ステージ前のモノローグ）
- ポーズ機能（初期値X。戦闘・コンボ時間を停止し、再開・設定・タイトルを選べる）
- 設定画面（音量調整、ゲーム操作とメニュー決定・戻るキーの変更・初期化）
- ゲームオーバー画面
- エピローグ（`STORY_BEATS` のエピローグ台本をタイプライター表示）
- ゲームクリア画面
- ハイスコア画面
...
| Mixin | 責務 |
|---|---|
| DebugMixin | デバッグコマンド（F1〜F8、Ctrl+1〜9: ステージワープ）。F8で情報表示を切替。通常章は既定非表示、stage 99は表示 |
...
| `LaserBeam` | entities/laser_beam.py | 連続レーザー（6段階、状態機械、3層描画） |
...
| `EnemyBilly` | entities/enemies/billy.py | 低速・高HP・回復1個、Wは配置指定 |
...
class Weapon:
    ...
    speed_level:  int   # 0〜3（+25%/段、最大1.75倍）
...
アクションの核は「予告を読む → 避ける → 火力を通せる位置へ戻って反撃する」。
戦闘中の攻撃操作は通常射撃とレーザーの2つ。持ち駒システムは廃止した。

- **レーザー**: 押している間だけ撃ち、離すと直ちに攻撃判定が止まる。充填・残弾ゲージなし。
  熱だけを制約とし、発射中48/秒で加熱、停止中32/秒＋先輩の解熱弾Lv×3で冷却。
  上限で2.4秒停止して40へ戻る。通常弾と追尾弾は常に使える。
- **反撃の隙**: 敵が止まっている間は被ダメージ2倍、レーザー中も冷却2倍。
  ブロリー両形態は大技が消えた後2.4秒だけ弱点が開く。防御中も20%のダメージは通る。
  体幹ゲージを持たず、攻撃を避けて正面へ戻ることを攻略にする。
  通常形態の予告は0.95秒、超サイヤ人は1.7秒。後者のビーム高260px、吸引125px/秒。
  本編の地形内に退避場所を残す。同じビームでの被弾は1回で、表示と寿命は続く。
- **体幹**: 1面・藤井Form1はシールド解除中に削れる。体幹が残っていてもHPへ通常ダメージを通す。
  藤井Form2は装甲を削り切るとダウンする。
  連射の小ダメージも防御倍率を適用し、小数を蓄積して切り捨てによる無効化を防ぐ。
  Form3は物語の進行が主役で体幹対象外。
- **子機**: 3面は開始から子機で防御。全機を倒すと2.6秒停止。2組倒すと盾が壊れ、落石を使う本体戦へ移る。
  レーザーなしでも前の2機から奥の1機へ進める。
- **成長**: 主砲は正面の弾を残して範囲・連射を増やす。レーザーは正面の集中火力、
  追尾弾は2ダメージの補助攻撃。速度は初期140px/秒から3段階で175・210・245px/秒へ成長する。
- **コンボ**: 得点だけの任意の遊び。別の在庫や入力を発生させない。
- **症状悪化**: 長期戦で通常射撃の間隔を最大1.4倍へ短縮。大技の予告・後隙を短縮しない。

具体的な測定・改善履歴は `docs/action_core_review_20260919.md`、数値は `balance.py`、
武器は `weapon.py` / `laser_beam.py`、攻撃は `boss.py`、敵配置は各章JSONを参照する。
...
`weapon_gate` は報酬用の血栓ゲートで、未指定では `fixed_drop: "WeaponItem"` として扱われ（明示的な `null` は報酬なし）、内部に青白い報酬コアを描く。
...
以下の `data/` は開発時の例。実際の書込先は `user_data_dir()` に統一する。
配布EXEは起動フォルダやEXE配置先の書込可否にかかわらずOSのユーザーデータ領域を使い、Windowsでは `%APPDATA%\InfuruToNoShito\` に保存する。
...
# → build-reports/packaged-smoke.json
```

EXEの `--smoke-test` を一時フォルダから起動し、全章の地形・マスク読込、初期化・描画、次章判定、隔離したユーザー領域での設定・ハイスコア・プレイログの保存と再読込を検査する。
通常のプレイヤー保存データは使わない。EXEやレポートの場所を変える場合は `--exe` / `--report` を指定する。
これは配布物のヘッドレス検査であり、実ウインドウの操作や全編クリアを確認したことにはならない。

### 9.4 リリース自動化（GitHub Actions）

`v` で始まるタグを push すると、`.github/workflows/release.yml` が Windows ランナーで
対象コミットの構文・整合性・テストを確認し、EXEをビルド → `tools/run_packaged_smoke.py` による配布物検査 → zip化 → GitHub Releaseへ添付する。検査結果のJSONもArtifactsへ保存する。
...
| S4 先輩復帰（緑の光と軌跡） | `bgm/Rebirth_the_edge.mp3` | カロナール先輩復帰の盛り上がりで切替（音量は基準の0.7倍） |
