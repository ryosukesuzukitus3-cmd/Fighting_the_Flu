...
この節は Codex 固有の追記であり、下の AUTOGEN 共有ガイドに Claude 向けのモデル名や運用例がある場合でも、Codex ではこの節を優先して読み替える。

Codex のメインエージェント（例: GPT-5.5）は、必要に応じて自分で手を動かしてよいが、原則として計画・判断・統合・最終確認に集中する。トークンやコスト削減を優先する場合は、軽量モデル（例: GPT-5.4 / GPT-5.4-mini）やサブエージェントを作業員・調査員・レビュー補助として使う。ただし、サブエージェントは最終責任者ではない。

メインエージェントが責任を持つこと:
- 最終計画と作業分解
- 設計判断、SSOT 判断、実装方針の決定
- branch / worktree 名と base の決定
- 未コミット変更やユーザー作業を巻き込まないことの確認
- 中〜高リスク実装の統合とレビュー
- キャプチャ・検証結果の最終判断
- PR 本文の最終承認
- マージ状態の解釈
- worktree / branch / remote branch の削除対象確認

軽量モデルやサブエージェントに向く作業:
- worktree 作成、環境構築、検証コマンド実行などの定型作業
- 範囲を限定したコードベース調査
- 既存パターン確認
- 簡単な機械的修正
- キャプチャ取得と一次確認
- PR 本文の下書き
- 対象 PR / branch / worktree が明示された後片付けコマンド実行

委譲するときは、対象ファイル・コマンド・観点を狭く指定し、返答は結論・根拠・リスクに絞らせる。広域探索を複数のサブエージェントへ丸投げしない。メインエージェントが結局すべて再確認し直す必要がある委譲は避ける。
