...
口調メモ:
  ...
  - 先輩は説教しない。含蓄が出そうになったら、自己都合（「俺が気まずい」）・
    手柄の主張（「褒めろにょ」）・無責任（「全部想像で言ったにょ」「パッケージに
    書いてあった」）でクズ寄りに着地させる。ポンコツ（くじで負けて担当等）。
    ただし芯の場面（喪失・復帰・終局）だけは締める。
  - 先輩には「薬物疑惑」がある（ヤニネコ／ヤクネコ路線）。本編では基本
    シラフだが、妙に"そっち"の知識が豊富で（毒の抜き方・効きすぎの経験談）、
    出所を聞かれると挙動不審になる（「……なんでだろうにょ」）。不必要に
    合法性を主張する。疑惑は疑惑のまま、絶対に確定させない。
...
PROLOGUE: list[Page] = [
    ...
    page(N, "肩書だけ並べれば、そこそこ戦えそうな人生だった。",
            "なのに今夜は、インフルエンザ一匹に完敗している。", fx=("shake", "red_noise")),
    # ── 発熱の予兆（？？？＝熱そのものの声）──
    page(N, "寒気がする。",
            "部屋の隅で、熱が、形をとっていく。"),
    page(Q, "……ぞくっと、きただろう。",
            "それが合図だ。今夜は、おれの番だ。", fx=("red_noise",)),
    page(Q, "眠れ。考えるな。",
            "熱の夜の考え事に、ろくなものはない。", fx=("shake",)),
    # ── 澤口、驚く（状況把握）──
    page(S, "……っ、誰だ。ここは……どこだ。",
            "天井も、壁もない。"),
    page(S, "夢か。",
    ...
    page(K, "ただの夢じゃないにょ。あいにくにょ。",
            se="SE_KARONARU_ARRIVE", fx=("white_particle",)),
    # ── 澤口、さらに驚く → 疑う（喋る薬を即受容しない）──
    page(S, "うわっ……光る錠剤が、しゃべってる。",
            "熱で、見えちゃいけないものまで見えてきた。"),
    page(K, "幻覚扱いとは、失礼なやつだにょ。"),
    page(K, "信じなくていいにょ。効くから。"),
    # ── 先輩、応じる（初出の名乗りはここへ）──
    page(K, "にょはカロナール先輩だにょ。今夜のお前の担当だにょ。",
            "あいにく、外れの夜だにょ。",
            "朝には溶けるにょ。残業は受け付けないにょ。"),
    page(S, "担当？ 勝手に決めるな。帰ってくれ。"),
    page(K, "文句は処方箋に書けにょ。"),
    page(K, "あと、にょも好きで来たんじゃないにょ。",
            "薬箱のくじで負けたにょ。"),
    page(S, "……クズの気配がする。"),
    page(K, "ここはお前の発熱夢だにょ。",
            "熱で見なくていいものが、",
            "全部でかい顔で立ってる場所だにょ。"),
    page(S, "最悪のテーマパークだな。"),
    page(K, "安心しろにょ。この夢は、合法だにょ。"),
    page(S, "何と比べてるんだ。"),
    page(K, "入場料は38.9℃だにょ。",
            "……で、行くのか、寝るのか。",
            "にょはどっちでも、横にいるにょ。"),
    page(S, "……行く。寝て覚める熱なら、とっくに引いてる。",
            "なんなら明日、出社する。"),
...
_BRIEF_STAGE1: list[Page] = [
    ...
    page(S, "雑だな。"),
    page(S, "……ところで、薬に頼りすぎるのも、",
            "どうかと思うんだが。"),
    page(K, "おい。今のは聞き捨てならんにょ。",
            "薬は、用法・用量を守れば、友達だにょ。"),
    page(S, "目が据わってるぞ。", last=True),
]
...
FINAL_SEQ: dict[str, list[Line]] = {
    ...
    "sengen": [
        ...
        Line(S,              "避けられない……。"),
        Line(BOSS_SAWAGUCHI, "これは外から飛んでくる弾じゃない。",
                            "お前の内側から聞こえる、お前の声だ。"),
    ...
    # SCENE095 最終フェーズ開始
    "act2_start": [
        Line(S,   "……先輩、頼む。今度は、一緒に。"),
        Line(SYS, "HP回復。抗反芻フィールド展開。頑固王の再生を停止。"),
        Line(KM, "にょにできるのは、痛みを少し遠ざけることだけだにょ。"),
...
_TALK_1_2: list[Page] = [
    ...
    page(S, "最悪だ。"),
    page(K, "うん。毒だにょ。"),
    page(S, "まあ、俺はSNSを見ない主義だから、関係ないが。"),
    page(K, "お前、さっき熱にうなされながら",
            "3時間見てたにょ。"),
    page(S, "あれは情報収集だ。"),
...
_BLACKHOLE: list[Page] = [
    ...
    page(K, "最初からそう言えにょ。"),
    page(S, "まずい。事象の地平面が、広がってる。", fx=("shake", "blackhole", "bh_pull")),
    page(K, "入るなにょ。"),
    page(S, "もう吸い込まれてる。"),
    ...
    page(K, "なるほどにょ。",
            "それは、かなり嫌だにょ。"),
    page(K, "澤口。作用反作用って、知ってるかにょ。", fx=("bh_resolve",)),
    page(S, "馬鹿にするな。",
            "ニュートンの第三法則だろ。"),
    page(K, "じゃあ、話が早いにょ。"),
    page(S, "待て。",
            "お前、何を——"),
    page(S, "やめろ！",
            "その向きに蹴ったら、反作用でお前が落ちる！", fx=("shake", "bh_push")),
    page(K, "薬ってのはにょ。", fx=("bh_fall",)),
    page(K, "先に溶けるものなんだにょ。"),
    page(S, "カロナール先輩！"),
    page(S, "戻れ！",
            "事象の地平面を越えたら、情報は——"),
    ...
    page(S, "それは、未解決問題だ！"),
    page(K, "じゃあ、にょで実験しろにょ。物理学修士。"),
    page(K, "にょにできるのは、",
            "痛みを少し遠ざけることだけだにょ。", fx=("bh_hold",)),
    page(K, "でも、お前がもう一手指す時間くらいは、",
            "稼げるにょ。"),
    page(K, "……用法・用量は、守れにょ。", fx=("bh_farewell",)),
    page(S, "カロナール先輩……？", fx=("bh_gone",)),
...
EPILOGUE: list[Page] = [
    ...
    # ── ここから現実。現実の薬は、しゃべらない。──
    page(N, "枕元の薬箱に、手を伸ばす。"),
    page(N, "銀のシートの中で、緑の錠剤が、",
            "すまし顔で並んでいるだけだ。"),
    page(S, "……おい、先輩。"),
    page(N, "返事はない。"),
    page(N, "当たり前だ。現実の薬は、しゃべらない。"),
    page(S, "……そうか。そうだよな。"),
    page(S, "……また外れの夜が来たら、頼む。"),
    page(S, "今夜は、世話になったな。……すまにょ。"),
    page(N, "返事は、なかった。"),
    page(N, "——ただ、気のせいでなければ、"),
    page(N, "錠剤が少しだけ、得意げに見えた。", last=True),
]
