...
HEAT_MAX            = 100.0
HEAT_LASER_PER_SEC  = 42.0   # 継続レーザーの発熱。通常弾は常に使える
HEAT_PER_SHOT       = 0.0    # 通常弾を撃ち続けても成長の罰にはしない
HEAT_PER_LASER      = 10.0   # レーザー1発射あたり
HEAT_COOL_RATE      = 22.0   # レーザー停止中の基礎冷却（/秒）
HEAT_COOL_KARONARU  = 3.0    # 先輩・解熱弾Lv1につき加算される冷却（/秒）
HEAT_BOSS_DOWN_MULT = 2.0    # ボスダウン中の冷却倍率（＝ダウン中は全部撃ってよい）
OVERHEAT_DURATION   = 1.6    # レーザーの冷却ロック時間（秒）
HEAT_AFTER_OVERHEAT = 30.0   # 熱暴走明けの体温（すぐ再過熱しない程度に残す）
